...
export default function Create({ auth, facilitytype }) {
    const { data, setData, post, errors, reset, progress } = useForm({
        title: facilitytype.data.title,
        start_date: facilitytype.data.start_date,
        end_date: facilitytype.data.end_date,
        _method: "PUT",
    });

    const onSubmit = (e) => {
        e.preventDefault();
        post(route("facilitytype.update", facilitytype.data.id));
    };
    return (
        <Authenticated
            user={auth.user}
